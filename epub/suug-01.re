...
DNSは、このインターネットの世界で最も広く成功している広域分散DBです@<fn>{suug-03}。
...
//footnote[suug-03][この本に書くためのこじつけ完了]
...
//footnote[suug-04][DNS Amp攻撃とかは別系統なのでここでは取り上げません]
...
//image[udp-header][UDP Header Format]{
//}

//image[dns-header][DNS Header Format]{
//}

UDP パケットは RFC768 @<fn>{suug-01}
で定義されているように、ポート番号(16bit)と長さ・チェックサムしかない
極めて簡素なパケットです（@<img>{udp-header}）。
またDNSパケットのヘッダフォーマットは RFC 1035 @<fn>{suug-02}
にあります。
DNS パケットには、クエリ自体に関する情報以外に 16bit の Query ID が入っています（@<img>{dns-header}）。
//footnote[suug-01][http://tools.ietf.org/html/rfc768]
//footnote[suug-02][http://tools.ietf.org/html/rfc1035]
...
経験がある方もいらっしゃるでしょうが、TCPにはTIME_WAIT状態があり、
...
//footnote[suug-05][DNSのパケット長は歴史的経緯により512バイトに制限されており、DNSSECのような長大なメッセージを伝えるのに不向きでした]
...
これを受けてか、 1997年7月発行の RFC2181@<fn>{suug-06} には、
...
//footnote[suug-06][http://tools.ietf.org/html/rfc2181]

とはいえ、優先度は定めても、そのデータを何に使って良いかは依然として書かれていません。
具体的に、キャッシュ優先度を並べてみると、以下のようになっています。

  * Primary zone file から得られた情報（ただし Glue は除く）
  * Zone transfer から得られた情報（ただし Glue は除く）
  * 権限のある回答の Answer セクションの権限ある情報
  * 権限のある回答の Authority セクションの情報
  * Primary zone や zone transfer に含まれる Glue
  * 権限のない回答の Answer セクション、または権限のある回答の Answer セクションの権限のない情報
  * 権限のない回答の Authority セクションの情報、または（権限のあるなしにかかわらず）Additional セクションの情報
...
このキャッシュ回避手法は、狭義のカミンスキーアタックとも言われる場合がある@<raw>{|html|<sup>［要出典］</sup>}
ほどに特徴的なものなのです。
