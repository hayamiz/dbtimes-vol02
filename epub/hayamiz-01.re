...
記事の中に要点のリストがあるので，拙訳とともに引用します：
  ...
  4. Missing most of the features that are routinely included in current DBMS @<br>{} 現代のDBMSがごく当たり前に持っている機能の殆どが欠けている
  5. Incompatible with all of the tools DBMS users have come to depend on @<br>{} DBMSユーザが使っているツールのほとんど全てと互換性がない

この記事を書いたのはDavid J. DeWitt と Michael Stonebraker @<fn>{fn00}の2人です。
...
//footnote[fn00][RDBMSを世界で最初に実装したStonebrakerの功績は The Database Times vol.1 を参照のこと]
...
だけに集中することができます。@<fn>{fn01}
どの索引を使ってどのファイルからデータを読み込むのか，
ネストループ結合をつかうのかハッシュ結合を使うのか，
ソートはオンメモリでやるか外部ソートを使うか，
こういったデータ処理のための@<em>{手続き}からプログラマを解放したのがSQLなのです。
//footnote[fn01][もちろん，DBMSの中で具体的な処理がどのように行われているのか，アプリケーションプログラマも知っているにこしたことはありません。ただし，「知らなくても使える」というのは重要なことです。]
...
最後に，DeWittとStonebrakerのMapReduce批判をもう一度引用します。
  ...
  3. 全く新規性に欠ける -- 25年近く前に開発された手法の限定的な実装でしかない
  4. 現代のDBMSがごく当たり前に持っている機能の殆どが欠けている
