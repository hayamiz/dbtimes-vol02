...
== データベース業界からのMapReduce批判

2009年，Web上にある記事が公開されて大きな話題を呼びました。
@<raw>{|html|&ldquo;}MapReduce: A major step backwards@<raw>{|html|&rdquo;} と題されたその記事は，
...
記事の中に要点のリストがあるので，拙訳とともに引用します：
  ...
  1. A giant step backward in the programming paradigm for large-scale data intensive applications @<br>{} 大規模データインテンシブアプリケーションにおけるプログラミングパラダイムの大いなる後退
  2. A sub-optimal implementation, in that it uses brute force instead of indexing @<br>{} 索引を使わない総当たり方式の，およそ最善とは言いがたい実装
  3. Not novel at all - it represents a specific implementation of well known techniques developed nearly 25 years ago @<br>{} 全く新規性に欠ける - 25年近く前に開発された手法の限定的な実装でしかない
  4. Missing most of the features that are routinely included in current DBMS @<br>{} 現代のDBMSがごく当たり前に持っている機能のほとんどが欠けている
  5. Incompatible with all of the tools DBMS users have come to depend on @<br>{} DBMSユーザが使っているツールのほとんど全てと互換性がない

この記事を書いたのはDavid J. DeWitt と Michael Stonebraker@<fn>{hayamiz-01-fn-1}の2人です。
...
//footnote[hayamiz-01-fn-1][RDBMSを世界で最初に実装したStonebrakerの功績は The Database Times vol.1 を参照のこと]
...
//footnote[hayamiz-01-fn-2][もちろん，DBMSの中で具体的な処理がどのように行われているのか，アプリケーションプログラマも知っているにこしたことはありません。ただし，「知らなくても使える」というのは重要なことです。]
...
== Hadoop使ったら負けかなと思っている

最後に，DeWittとStonebrakerのMapReduce批判をもう一度引用します。

  1. 大規模データインテンシブアプリケーションにおけるプログラミングパラダイムの大いなる後退
  2. 索引を使わない総当たり方式の，およそ最善とは言いがたい実装
  3. 全く新規性に欠ける ー 25年近く前に開発された手法の限定的な実装でしかない
  4. 現代のDBMSがごく当たり前に持っている機能のほとんどが欠けている
  5. DBMSユーザが使っているツールのほとんど全てと互換性がない
