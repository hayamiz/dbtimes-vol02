...
//image[pgday-water][▲受付を済ませると，発表資料に加えてJPUGイベントではお馴染みの「かめすい」がもらえる]
...
「今日この場にいる皆さん自身もPostgreSQLコミュニティの一員である(@<raw>{|html|&ldquo;}You are PostgreSQL.@<raw>{|html|&rdquo;})」
ということでした。
...
記事や書籍を英語に@<raw>{|html|&ldquo;}逆輸入@<raw>{|html|&rdquo;}することで，
...
@<raw>{|html|&ldquo;}プロ@<raw>{|html|&rdquo;}としてプライドがある人ならばこのくらいできて欲しいよね，
という話をします」
...
例えば，EnterpriseDB社@<fn>{haya-01}ではPostgreSQLの機能強化版であるPostgreSQL Plusを販売していますが，
PostgreSQL Plusの大きな特徴はOracleとの互換性，つまりOracleからの移行の容易性です。
Oracleと比べて足りない機能は，PostgreSQL開発ロードマップの重要な指針であることは間違いありません。
//footnote[haya-01][EnterpriseDB社にはコアチームメンバを始めとしたPostgreSQL開発の中心的人物が数多く所属している。]
...
PostgreSQLユーザからの要望調査第1位にある機能です@<fn>{haya-02}。
//footnote[haya-02][http://postgresql.uservoice.com/forums/21853-general]
//footnote[haya-05][ちなみに，IBMのDB2では索引を張っていないカラムの集計もIndex Only Scanで出来るよう，索引に指定した属性の値を同居させる機能を持っていたりします]
...
面白い機能もリリースされて話題を呼びました@<fn>{haya-03}。
さらに，2010年末にHerokuを買収したSalesforceが，
PostgreSQLのメーリングリストにPostgreSQL技術者を募集するメールを投稿したことから，
SalesforceがOracleからOSSへシフトするのか？
と話題を呼びました@<fn>{haya-04}。
//footnote[haya-03][https://postgres.heroku.com/blog/past/2012/11/8/fork_your_data/]
//footnote[haya-04][http://archives.postgresql.org/pgsql-jobs/2012-10/msg00003.php]
